-- Audit Logs Database Views Setup - FINAL CORRECTED VERSION
...
-- SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'customer_debit_audit_logs';

-- 1. Create combined_audit_logs view (based on actual data structure)
...
-- 4. Create indexes for better performance (only for existing columns)
...
-- 5. Test queries (run these to verify everything works)
